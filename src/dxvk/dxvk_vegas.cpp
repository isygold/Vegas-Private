...
namespace dxvk {
  ...
  // Framegen pending-drain list (fence-timeout path): when vkWaitForFences
  // times out the submitted CB may still be executing; destroying
  // fence/pool/cmdBuf/views immediately is UB and can wedge the queue
  // (hard freeze on Adreno 610 after repeated VK_TIMEOUT).  Parked here,
  // destroyed on a later dispatch once their fence signals.
  ...
  // Watchdog for the warp blend policy: net slow main-dispatch fence
  // waits (> 25ms), capped at 5 -> forces blend floor 0.95 (present
  // mostly-current frame).  Clean waits decay by one (oscillation-tolerant).
  static uint32_t           s_fgSlowCount    = 0;

  // Saturation-skip (adaptive dispatch): when s_fgSlowCount trips,
  // framegenDispatch returns early (native present) instead of stalling
  // the present thread on a saturated queue.  After FG_SKIP_WINDOW
  // skipped frames a re-arm probe runs one dispatch to test recovery.
  static constexpr uint32_t FG_SKIP_WINDOW   = 60;
  static uint32_t           s_fgSkipFrames   = 0;

  // Framegen bimodal-gate stats (set 1 / binding 0): single 16B coherent
  // buffer, own layout+pool.  Buffer+set always created so the motion
  // shader atomicAdds have a valid target; readback gated OFF in beta.
  ...
  // Zero-pad to exactly 3 digits ("97" -> "097") for stat log means.
...
*     mirroring DXVK's own LibraryLoader. The old dlopen-only path could
     *  never work in the PE build, so the loader stayed dead on Android. */
...
Vegas::s_fgStatsEnabled = false;  // BETA STUB: GPU-side stats readback disabled (host CSV ring is the telemetry home)
      // (buffer+set still created below so the motion shader atomics have a valid target)
  ...
  bool Vegas::isFrameGenReady() {
    ...
    auto dev = s_dxvkDevice;
    if (dev != nullptr && dev->config().vegasEnableFramegen == Tristate::False)
      return false;
  ...
  void Vegas::recordDrawCall(uint32_t count) {
    ...
    // Draw-count CSV profiling (beta): no-op when inactive. Batch paths
    // pass the number of draws so multi-draw is not undercounted.
    if (!s_profileActive)
      return;
  ...
  void Vegas::pushMetrics(
    ...
    // reads the process-shared config/env directly (dxvk.conf parses
    ...
    if (csvActive) {
      ...
      if (shared != nullptr) {
        ...
        static bool s_csvMapWarned = false;
        if (!s_csvMapWarned) {
          s_csvMapWarned = true;
